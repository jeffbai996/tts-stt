...
- Recording audio from mic or Discord voice channels (separate concern, pulls in `botA-voice` scope).
...
The repo has no README currently. This spec does not require creating one — repo hardening (README, tests for existing code, cleanup of the empty `repos/botC-tts/` subdir, v0.1 tag) is a separate follow-up task, not part of this work. The `.env.example` additions document the STT knobs sufficiently for now.
